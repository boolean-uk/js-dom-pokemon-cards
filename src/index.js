...
for(i = 0; i < data.length; i++){
    ...
    console.log('src', data[i].sprites.other["official-artwork"].front_default)
    card.append(image)
    ...
    // unorderedList.setAttribute('class', 'card--text')
    for (let j = 0; j < data[i].stats.length; j++) {
        ...
        console.log('innerText', data[i].stats[j].stat.name.toUpperCase() +': '+ data[i].stats[j].base_stat)
        card.append(stats)
    }
