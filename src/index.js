...
const viewOfCards = (pokemon) => {
  ...
  const generateNames = () => {
    const versionList = game.map(
      (index) => `<li>${capitalizeFirstLetter(index.version.name)}</li>`
    )
...
pokemonElements.forEach((card) => {
  ...
  imageElement.addEventListener("click", () => {
    ...
    const artworkIndex = parseInt(
      pokemonElements.getAttribute("data-current-artwork"),
      10
    )
    ...
    pokemonElements.setAttribute(
      "data-current-artwork",
      nextArtworkIndex.toString()
    )
  })
})
